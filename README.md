...
## 数据与服务
- 停车场：© OpenStreetMap 贡献者，ODbL 1.0。全菲 `amenity=parking` 与 `amenity=motorcycle_parking`，剔除 `access=private/no`。数据时间见页面底部与 `data/parking.json` 的 `meta.osm_base`。不含实时空位与价格。
...
## 验收
`node tools/accept.mjs`（本地）、`--url https://jyb635050-ai.github.io/parking-ph/`（线上）、`--prove`（屏蔽算路，必须失败）。判卷脚本冻结，SHA256 `7F7E13FB4AB87F347DB331658C4E549B9289FF7914FD11A3C17F1FD1088AB5ED`。
